...
class _DoctorScreenState extends State<DoctorScreen> {
  ...
  // Higher number = more urgent = should appear first
  ...
  Future<void> _loadCases() async {
    ...
    // Sort: unconfirmed cases before confirmed ones, then by severity
    // (Category III first), then newest first within the same severity.
    cases.sort((a, b) {
      final aConfirmed = a['confirmed'] == true;
      final bConfirmed = b['confirmed'] == true;
      if (aConfirmed != bConfirmed) {
        return aConfirmed ? 1 : -1; // unconfirmed cases first
      }
      ...
      return bCreated.toString().compareTo(aCreated.toString()); // newest first
    });
